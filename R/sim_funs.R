...
#'
#' @param theta vector of (sample) effect sizes
...
#'
#' This function estimates the probability of seperiority to control (first arm) for each treatment arm from a posterior sample of effect sizes
#'
#' @param theta vector of (sample) effect sizes
#' @return vector of probabilities of superiority to control - first component is always zero
#'
#' @export

control_sup_check = function(theta, con = 1) {
  n = length(theta)
  check = c()
  for (i in (1:n)[-con]) check[i] = (theta[i] > theta[con])
  check[con] = 0
  return(check)
}
...
#' @param adapt logical; if TRUE randomization probabilities are adapted
#' @param con logical; if TRUE the first arm is treated as control
#' @param prand vector of initial randomization probabilities
#' @return updated posterior samples
#'
#' @export

RAR_sim = function(nt, theta0, nb = 1, maxN = 500, N = 1000, upper = 0.95, lower = .01,
                   burn = 10*nt, response.type, conjugate_prior = T, padhere = rep(1,nt), adapt = T,
                   con = T, prand = rep(1/nt, nt), control_sup = F) {
  ng = nb
  j = 0
  ...
  repeat {
    ...
    if (adapt == T) prand = sqrt(psup[,j]) else {
      ze = which(psup[,j] == 0)
      prand = rep(1/np, nt)
      if (!is.null(ze)) prand[ze] = 0
    }
    if (con == T) prand[1] = 1/np
    ...
    con_check = t(apply(theta[,,j+1], 1, control_sup_check))
    if (length(y) < burn) {
      psup = abind(psup, psup[,j], along = 2)
    } else { 
      if (control_sup == T) psup = abind(psup, apply(check, 2, mean), along = 2) else psup = abind(psup, apply(control_sup_check, 2, mean), along = 2)
    }
    psup[psup[,j+1] < lower | psup[,j] == 0,j+1] = 0
    if (control_sup == T) psup_out = abind(psup_out, apply(check, 2, mean), along = 2) else psup_out = abind(psup_out, apply(check, 2, mean), along = 2)
  ...
  out = list(psup = psup, theta = theta, est = est, y = y, x = x[,-1], x0 = x0[,-1])
  class(out) = 'trial'
  return(out)
}
...
sim_wrapper = function(i, nt, theta0, nb = 1, maxN = 500, N = 1000, upper = 0.95, lower = .05,
                       burn = 10*nt, response.type, conjugate_prior = T, padhere = rep(1,nt), adapt = F, 
                       con = T, prand = rep(1/nt,nt)) {
  ng = nb
  j = 0
  x = array(0, dim = c(nt, 1))
  y = NULL
  ...
  ## any other output for any single trial that you'd like to keep track of
